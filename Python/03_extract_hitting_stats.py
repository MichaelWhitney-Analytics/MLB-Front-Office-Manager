...
def get_hitting_stats(season: int) -> dict:
    """Retrieve MLB regular-season individual hitting statistics."""
...
def main() -> None:
    hitting_data = get_hitting_stats(SEASON)
    output_file = save_json(hitting_data, f"hitting_stats_{SEASON}.json")

    splits = hitting_data.get("stats", [{}])[0].get("splits", [])
    player_count = len(splits)

    print(f"Successfully extracted hitting statistics for {player_count} players.")
