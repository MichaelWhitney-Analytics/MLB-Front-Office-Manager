...
def get_team_pitching_stats(team_id: int, season: int) -> dict:
    """Retrieve regular-season pitcher statistics for one MLB team."""
    response = requests.get(
...
def main() -> None:
    ...
    for index, team in enumerate(teams, start=1):
        ...
        try:
            stats_data = get_team_pitching_stats(team_id, SEASON)
            splits = stats_data.get("stats", [{}])[0].get("splits", [])

            team_pitching_data.append(
                {
                    "team_id": team_id,
                    "team_name": team_name,
                    "stats": stats_data,
            ...
            print(
                ...
                f"Retrieved {len(splits)} pitcher records for {team_name}."
            )
    ...
    output_data = {
        "teams": team_pitching_data,
        "failed_teams": failed_teams,
    }

    output_file = save_json(
        output_data,
        f"team_pitching_stats_{SEASON}.json",
    )

    total_pitcher_records = sum(
        len(team_data["stats"].get("stats", [{}])[0].get("splits", []))
        for team_data in team_pitching_data
    )

    print()
    print(f"Successfully retrieved data for {len(team_pitching_data)} teams.")
    print(f"Failed team requests: {len(failed_teams)}.")
    print(f"Total player-team pitching records: {total_pitcher_records}.")
